...
if ! ip link show "$IFACE" &>/dev/null; then
  ...
  # get default route interface (usually eth0 if col0 is missing)
...
COL0_IP=$(ip -o -4 addr show dev "$IFACE" \
...
echo "   namespaces: $PLT_NS, $INF_NS, $XAPP_NS"
...
for ns in "$PLT_NS" "$INF_NS" "$XAPP_NS"; do
...
# deploy ORANSlice xApp
echo "📦 Installing ORANSlice xApp..."
cd /root/ric-plt-appmgr/xapp_orchestrater/dev/xapp_onboarder/charts || {
  echo "❌ cannot cd into xapp_onboarder/charts"
  exit 1
}

helm install oranslice-xapp oranslice-xapp-1.0.0.tgz -n "$XAPP_NS" \
  --set global.image.pullPolicy=IfNotPresent \
  --wait --timeout 5m
...
echo "→ $XAPP_NS pods:";kubectl get pods -n "$XAPP_NS" -o wide